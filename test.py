...
def _check_tied_weights(model, tie_word_embeddings: bool):
    """Check if weights are tied correctly."""
    if tie_word_embeddings:
        if not hasattr(model, "lm_head") or not hasattr(model.model, "embed_tokens"):
            return False, "Modules missing"
            
        embed_w = model.model.embed_tokens.weight
        lm_head_w = model.lm_head.weight
        return embed_w is lm_head_w, "Not tied"
    return True, "No tie needed"
...
def test_fsdp2_meta_loading_buffers_and_tied_weights():
    ...
    normal_tied_ok, _ = _check_tied_weights(normal_model, tie_word_embeddings)
    ...
    meta_tied_ok, _ = _check_tied_weights(meta_model, tie_word_embeddings)
    ...
    # 1. Non-persistent buffers (e.g., inv_freq)
    ...
    # 2. Tied weights (embed_tokens.weight and lm_head.weight)
    if tie_word_embeddings:
        assert normal_tied_ok, "Normal model failed to tie weights"
        assert meta_tied_ok, "Meta model failed to tie weights after load"

...
"""Trainer callback primitives.

Core classes:
  - TrainerState    – read-only snapshot of training progress.
  - TrainerCallback – base class; override any hook you need.
  - CallbackHandler – fan-out dispatcher that owns a list of callbacks.

Callbacks are pure *observers*: they can read training state and perform
side-effects (logging, uploading, profiling …) but they do NOT control the
training loop.  Scheduling decisions (when to log, when to save, when to
stop) belong to the trainer itself.
"""
...
class TrainerCallback:
          ...
          for each epoch:
            on_epoch_begin
              for each step:
                ...
                [on_save]  ← if this step is a saving step
            on_epoch_end
        on_train_end
    """
    ...
    def on_log(self, args: "TrainingArguments", state: TrainerState, logs: dict[str, Any], **kwargs: Any) -> None:
        ...
        Args:
            logs: Dict of metric names → values for the current step.
        """
...
# ---------------------------------------------------------------------------
# Callback handler (fan-out dispatcher)
# ---------------------------------------------------------------------------
